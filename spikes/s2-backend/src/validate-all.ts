  ...
  try {
    ...
    for (const role of roles) {
      ...
      if (role === 'superadmin') {
        ...
        if (!canManageAll) throw new Error('superadmin should manage FinanceDashboard');
      ...
      if (role === 'finance') {
        ...
        if (!canRead) throw new Error('finance should read FinanceDashboard');
        if (canSync) throw new Error('finance should NOT manage FinanceSync');
        checks.push(`${role}:read=✓,sync=✗`);
      }

      if (role === 'sale') {
        const canRead = ability.can('read', 'FinanceDashboard');
        if (canRead) throw new Error('sale should NOT read FinanceDashboard');
      ...
      if (role === 'hr') {
        ...
        if (!canHR) throw new Error('hr should read HRData');
        if (canFinance) throw new Error('hr should NOT read FinanceDashboard');
      ...
      if (role === 'director') {
        ...
        if (!canRead) throw new Error('director should read FinanceDashboard');
        if (!canExport) throw new Error('director should export FinanceDashboard');
        if (canUpdateSettings) throw new Error('director should NOT update FinanceSettings');
...
function validateZod() {
  ...
  try {
    ...
    if (!valid.success) throw new Error('Valid input rejected: ' + JSON.stringify(valid.error));
    ...
    if (invalid.success) throw new Error('Invalid input accepted');
...
async function validateExpress5Integration() {
  ...
  try {
    ...
    const okData = await okRes.json() as { data: unknown[]; count: number };
    ...
    if (badPostRes.status !== 400) throw new Error(`Expected 400, got ${badPostRes.status}`);
    ...
    if (goodPostRes.status !== 201) throw new Error(`Expected 201, got ${goodPostRes.status}`);
    const created = await goodPostRes.json() as { data: { id: string } };
