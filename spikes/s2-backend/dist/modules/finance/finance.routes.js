...
router.get('/sales', authorize('read', 'FinanceDashboard'), financeController.getSalesDaily);
// POST /api/v1/finance/sales — requires FinanceDashboard create + Zod validation
router.post('/sales', authorize('create', 'FinanceDashboard'), validate(createSaleRecordSchema), financeController.createSaleRecord);
// GET /api/v1/finance/settings — requires FinanceSettings read
router.get('/settings', authorize('read', 'FinanceSettings'), financeController.getSettings);
