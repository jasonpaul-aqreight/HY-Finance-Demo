...
// Mock backend URL — in spike, we use a local API route
const backendUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api/mock'

export const authOptions: NextAuthOptions = {
  secret: process.env.NEXTAUTH_SECRET || 'spike-s1-secret-for-testing-only',
  providers: [
    // Primary: Phone number + password (mirrors HR production)
    CredentialsProvider({
    ...
    // Secondary: Email + password (backward compatibility)
    CredentialsProvider({
  ...
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      ...
      // Session update trigger (e.g., token refresh)
