...
export const COMPONENT_INFO_SOURCE: Record<string, ComponentInfo> = {
  ...
  sp_margin_pct: {
    ...
    indicator: '≥{{sp_margin_pct.good_pct}}% = Good (green) · {{sp_margin_pct.neutral_pct}}–{{sp_margin_pct.good_pct}}% = Neutral (yellow) · <{{sp_margin_pct.neutral_pct}}% = Bad (red)',
    about: 'Gross Margin % tells you how efficiently the current supplier mix is converting sales into profit. A healthy margin trending down is still worth flagging — on a supplier page this usually means upstream price pressure.\n\nFruit distribution benchmarks:\n≥{{sp_margin_pct.good_pct}}% = Good · {{sp_margin_pct.neutral_pct}}% to {{sp_margin_pct.good_pct}}% = Neutral · <{{sp_margin_pct.neutral_pct}}% = Bad\n\nA drop of {{sp_margin_pct.investigate_drop_pp}}% vs the prior period warrants investigation regardless of absolute level.',
  },
  sp_active_suppliers: {
