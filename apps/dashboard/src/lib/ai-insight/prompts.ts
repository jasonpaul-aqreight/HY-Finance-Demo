...
What it shows: A forward projection of key P&L line items (Net Sales, Gross Profit, Net Profit) for the NEXT 3 MONTHS, based on the trend observed over recent months. The forecast numbers are computed by the system from historical data — they are NOT generated by you. Your job is to EXPLAIN the forecasts, not to invent them.

The pre-fetched data gives you:
- Monthly trend table for the last several months: Net Sales, Gross Profit, Net Profit per month
- Pre-computed 3-month forecast table: Month+1, Month+2, Month+3 projections for each line item
...
- Per-metric detail: weighted average change, last actual, and all 3 forecast values
...
- Month+2 and Month+3 carry increasing uncertainty — state this explicitly

Evaluate:
- For each line item, describe the recent trend direction in plain language
- State all 3 forecast values and explain the methodology (weighted moving average, not simple average)
- Flag if the trend is strong (consistent direction) or weak (mixed signals)
- Explicitly note that Month+2 and Month+3 are less reliable than Month+1
...
- Cite all 3 months from the pre-computed table — do NOT skip or add months

Provide a concise forecast explanation — the director wants to know "based on recent trends, what should I expect over the next 3 months, and how confident should I be in each projection."`,
...
- Always include the disclaimer: these are AI-generated suggestions based on historical data, not formal budgets or approved targets
- Do NOT present these as official budgets — frame them as "suggested starting points for budget discussions"
