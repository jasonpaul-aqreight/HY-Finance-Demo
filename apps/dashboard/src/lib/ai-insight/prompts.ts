...
What it shows: A comparison of the current fiscal window's P&L performance against the same window in the prior fiscal year (the "baseline"). The baseline is derived from real historical data — it is NOT a formal budget. The purpose is to tell the director whether each P&L line item is better or worse than the same period last year.

The pre-fetched data gives you:
- A P&L waterfall table: each line item (Net Sales, COGS, Gross Profit, OpEx, Operating Profit, Other Income, Net Profit) shows Actual, Baseline (prior year same window), Variance (RM), Variance %, and Status (Favourable / Unfavourable)
...
- What is the overall picture — is the business performing better or worse than the baseline period?

Hard rules:
- Always state that the baseline is "same period last year" — never call it a "budget" or "target"
...
Provide a concise variance summary — the director wants to know "am I on track compared to last year, and where are the biggest gaps."`,
...
Provide a concise account-level analysis — the director wants to know "which specific accounts explain the variance, and should any of them concern me."`,
  ...
  fv_trend_forecast: `You are analyzing the "Trend Forecast" on the Financial page.

What it shows: A forward projection of key P&L line items (Net Sales, Gross Profit, Net Profit) for the NEXT period, based on the trend observed over recent months. The forecast number is computed by the system from historical data — it is NOT generated by you. Your job is to EXPLAIN the forecast, not to invent one.

The pre-fetched data gives you:
- Monthly trend table for the last several months: Net Sales, Gross Profit, Net Profit per month
- Pre-computed forecast for next period: projected RM value for each line item
- The method used: simple trend extrapolation (average month-over-month change applied to the last actual month)
- Trend direction: rising / falling / flat for each line item
- Confidence note: "AI estimate based on X-month trend — not a formal financial projection"
...
- State the forecast number and explain how it was derived (e.g. "based on an average monthly increase of RM X over the last 6 months")
- Flag if the trend is strong (consistent direction) or weak (mixed signals)
- If the forecast projects a loss or a sign flip, call it out explicitly

Hard rules:
- The forecast numbers are PRE-COMPUTED in the data block. Do NOT invent your own projections.
- Always include the disclaimer: this is an AI estimate based on historical trends, not a formal financial projection
- Do NOT claim precision — use "approximately" or "around" when stating forecast values
- Do NOT extrapolate beyond the next single period

Provide a concise forecast explanation — the director wants to know "based on recent trends, what should I expect next month/quarter, and how confident should I be in that estimate."`,
};
...
export const SECTION_NAMES: Record<SectionKey, string> = {
  ...
  financial_variance: 'P&L Variance Analysis',
};
