...
export function useKpisV2(filters: DashboardFiltersV2) {
  return useSWR<KpiV2Data>(
    filters.startDate ? `/api/payment/v2/kpis?${dateParams(filters)}` : null,
    fetcher,
...
export function useCollectionTrend(filters: DashboardFiltersV2) {
  ...
  const { data, ...rest } = useSWR<{ data: any[]; avg_pay_days: number | null }>(
    filters.startDate ? `/api/payment/collection-trend?start_month=${startMonth}&end_month=${endMonth}` : null,
    fetcher,
    { revalidateOnFocus: false },
  );
