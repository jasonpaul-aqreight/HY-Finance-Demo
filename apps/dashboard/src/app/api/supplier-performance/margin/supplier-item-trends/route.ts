...
import { getSupplierItemPriceTrends } from '@/lib/supplier-margin/queries';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await getSupplierItemPriceTrends(creditorCode, start, end);
    return NextResponse.json({ data });
  } catch (err) {
    console.error('supplier-item-trends error:', err);
