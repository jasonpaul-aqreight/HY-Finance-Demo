...
import { getProductCustomerMatrix, type MarginFilters } from '@/lib/customer-margin/queries';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const filters: MarginFilters = {
      ...
      productGroups: searchParams.getAll('group').filter(Boolean),
    };
    return NextResponse.json(await getProductCustomerMatrix(filters));
  } catch (err) {
    console.error('Product customer error:', err);
