...
import { getCustomerReturnDetails, getCustomerReturnDetailsAll } from '@/lib/return/queries';
...
export async function GET(req: NextRequest) {
  ...
  if (!startParam && !endParam) {
    const data = await getCustomerReturnDetailsAll(debtorCode);
  ...
  const data = await getCustomerReturnDetails(debtorCode, start, end);
  return NextResponse.json(data);
}
