      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0 space-y-2">
          ...
          <button
            ...
            className="flex items-center gap-1 text-xs font-medium text-blue-800 hover:text-blue-900"
          >
            {showRaw ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
            {showRaw ? 'Hide' : 'Show'} original feedback
          </button>
          {showRaw && (
            <div className="rounded border border-blue-100 bg-background p-2 text-sm whitespace-pre-line text-foreground/90">
              {row.rawFeedback}
            </div>
          )}
        </div>
...
export function FeedbackList({ promptKey, promptDisplayName }: Props) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const body = await res.json();
      if (!res.ok) {
        setDiffError(body.error ?? 'Failed to apply edit');
        return;
      }
      setDiffOpen(false);
      setDiffPayload(null);
