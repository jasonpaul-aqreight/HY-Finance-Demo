    ...
    <div className="mt-4">
      <div className="relative h-7 w-full overflow-hidden rounded-lg border border-border">
        ...
        <div
          ...
          style={{ width: `${highPct}%` }}
        />
        {/* Yellow zone: high to low */}
        <div
          className="absolute inset-y-0 bg-yellow-400"
          style={{ left: `${highPct}%`, width: `${Math.max(0, lowPct - highPct)}%` }}
        />
        {/* Green zone: low to 100 */}
        <div
      ...
      <div className="relative mt-1.5 h-5 text-xs">
        ...
        {lowPct > 5 && lowPct < 95 && Math.abs(lowPct - highPct) > 8 && (
          <span
            className="absolute -translate-x-1/2 text-emerald-600 font-medium"
            style={{ left: `${lowPct}%` }}
          >
            {low}
          </span>
      ...
      <div className="mt-3 space-y-2 text-sm">
        <div className="flex items-start gap-2">
          <span className="mt-1 inline-block h-3 w-3 shrink-0 rounded-sm bg-emerald-400" />
          <div>
            <span className="font-medium text-foreground">Low Risk ({low}–100)</span>
            <span className="ml-1.5 text-foreground/70">— All good. Customer pays on time and within limits.</span>
          </div>
        </div>
        <div className="flex items-start gap-2">
          <span className="mt-1 inline-block h-3 w-3 shrink-0 rounded-sm bg-yellow-400" />
          <div>
            <span className="font-medium text-foreground">Moderate ({high + 1}–{low - 1})</span>
            <span className="ml-1.5 text-foreground/70">— Watch closely. Follow up on overdue payments.</span>
          </div>
        </div>
        <div className="flex items-start gap-2">
          <span className="mt-1 inline-block h-3 w-3 shrink-0 rounded-sm bg-red-400" />
          <div>
            <span className="font-medium text-foreground">High Risk (0–{high})</span>
            <span className="ml-1.5 text-foreground/70">— Needs immediate attention. Consider pausing supply.</span>
          </div>
        </div>
      </div>
    </div>
...
function HowItWorks() {
  ...
  return (
    <Card>
      ...
      >
        <h3 className="text-base font-semibold">How Credit Health Score is calculated</h3>
        {open
      ...
      {open && (
        <CardContent className="space-y-6 border-t pt-4">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            The score is made up of 4 parts. Each part checks one thing about the customer.
            The higher the score, the safer the customer.
          </p>
          ...
          <div>
            ...
            <p className="mt-1.5 text-xs text-muted-foreground">
              * Customers with no credit limit set are not scored on this factor.
            </p>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              How long has the oldest unpaid invoice been overdue?
              The system picks the invoice that has been waiting the longest.
              Shorter wait = higher score.
            </p>
            <ScoreTable rows={[
              ['Nothing overdue', '100 (best)'],
              ['1 – 30 days late', '80'],
              ['31 – 60 days late', '60'],
              ['61 – 90 days late', '40'],
              ['91 – 120 days late', '20'],
              ['More than 120 days', '0 (worst)'],
            ]} />
            <p className="mt-1.5 text-xs text-muted-foreground italic">
              Example: Oldest unpaid invoice is 45 days overdue → Score = 60
            </p>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              When this customer pays, how many days late are they on average?
              Looks at the last 12 months of payments.
              Paying on time = highest score.
            </p>
            <ScoreTable rows={[
              ['On time or early', '100 (best)'],
              ['1 – 7 days late', '80'],
              ['8 – 14 days late', '60'],
              ['15 – 30 days late', '40'],
              ['31 – 60 days late', '20'],
              ['More than 60 days late', '0 (worst)'],
            ]} />
            <p className="mt-1.5 text-xs text-muted-foreground italic">
              Example: Customer pays on average 5 days late → Score = 80
            </p>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Has the customer gone over <strong>both</strong> their credit limit
              and overdue limit at the same time?
              This catches the most dangerous customers — those who are in trouble on two fronts.
            </p>
            <ScoreTable rows={[
              ['Neither or only one limit breached', '100 (safe)'],
              ['Both limits breached', '0 (danger)'],
            ]} />
            <p className="mt-1.5 text-xs text-muted-foreground italic">
              Example: Owes more than credit limit AND more than overdue limit → Score = 0
            </p>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Each factor&apos;s score is multiplied by its weight, then added up.
            </p>
            <div className="mt-2 rounded-md bg-muted/50 px-3 py-2 text-sm text-muted-foreground">
              <strong>Credit Score</strong> = (Utilization score x weight) + (Overdue score x weight) + (Timeliness score x weight) + (Double Breach score x weight)
            </div>
          </div>
...
export function SettingsDialog({ open, onClose }: SettingsDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto p-0" showCloseButton={false}>
        {/* Header */}
        <div className="bg-background border-b px-6 py-4">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            The Credit Health Score rates each customer from 0 (highest risk) to 100 (safest), shown in the Score and Risk columns below.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="px-6 py-4">
            ...
            <div className="space-y-4">
                ...
                <Card>
                  <CardHeader><CardTitle>Credit Health Score Weights</CardTitle></CardHeader>
                  <CardContent>
                  ...
                  <CardContent>
                    ...
                    {!thresholdsValid && (
                      <p className="mt-2 text-sm font-medium text-red-600">
                        Low Risk must be greater than High Risk
                      </p>
                    )}
        ...
        {/* Footer */}
        {isAdmin && (
        <div className="bg-background border-t px-6 py-3 flex items-center gap-3">
          {message && (
