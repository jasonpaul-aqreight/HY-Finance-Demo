...
test.describe('AI Insight Config after feedback removal', () => {
  ...
  test('AI Insight panel is expanded by default with no Analyze control', async ({ page }) => {
    await setAdminRole(page);
    await page.route('**/api/ai-insight/section/**', async (route) => {
      await route.fulfill({ status: 404, body: JSON.stringify({ exists: false }) });
    });

    await page.goto('/sales');
    await page.waitForSelector('button:has-text("Get Insight")', { timeout: 30_000 });

    await expect(page.getByText('No insights generated yet.').first()).toBeVisible();
    await expect(page.getByText('Generated by the admin AI Insight batch.').first()).toBeVisible();
