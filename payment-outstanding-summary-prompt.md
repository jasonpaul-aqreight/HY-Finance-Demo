# Outstanding Payment — Full Summary Prompt (sent to Sonnet 4.6)

Two messages are sent to the LLM: **SYSTEM** + **USER**. Below is the exact text for the `payment_outstanding` section.

> Source: SYSTEM + General + Rubrics rendered from local DB (2026-05-09, post compaction). Raw Data blocks captured live from orchestrator debug log of the 2026-05-07 15:39 production run (`apps/dashboard/logs/ai-debug-payment_outstanding-2026-05-07T15-39-17.log`). Component order shown is the canonical `SECTION_COMPONENTS` order — at runtime the parallel pool may complete components in any order, but the data values for each component are identical.
...
- Apply each component's Rubric block as the authority on good/neutral/bad — never invent thresholds.
...
- If a "General" block is provided, follow its instructions and answer its deterministic questions inside the Detail body. If it includes an "Output Override", apply that override in place of the Detail structure above.
```
...
Below is ABOUT and RAW DATA for each component in this section. 
- The About description nthe component role in the dashboard and the authority on what good/neutral/bad
- The Raw Data is what being pass to the dashboard and the number what user see. Every number must be traceable
