# AI Insight Engine — Full Stack Review (Per-Section)

**Date:** 2026-04-19
**Purpose:** Review every section's business question, components, data sources, thresholds, and business rules. Used to identify gaps, overlaps, and improvements before Phase B testing.
...
**Tool policy:** `none` → **changing to `aggregate_only`**
**Data source:** All components use `pc_ar_monthly`

| Component | Type | Key Metrics | Thresholds |
|-----------|------|-------------|------------|
| `avg_collection_days` | KPI | DSO per month, period avg, best/worst month | ≤30d Green, ≤60d Yellow, >60d Red |
...
| `collection_days_trend` | Chart | Monthly DSO line + period avg reference | Rising=bad, falling=good, spikes >60=critical |
...
**Key business rules:**
- `avg_collection_days` and `collection_days_trend` both calculate DSO from same data — intentional (KPI gives single number, chart shows monthly trend)
...
**Action items:**
- Rename DSO → Collection Days everywhere (code, SQL aliases, UI)
- Change tool policy from `none` to `aggregate_only`
...
**Tool policy:** `full` (can query local + RDS)
...
**Action items:**
- `customer_credit_health` fetcher: read weights + risk thresholds from DB instead of hardcoding
...
**Action items:**
- Combine `net_sales` + `invoice_sales` + `cash_sales` into one "Sales Summary" component
...
**Key business rules:**
- Has prior-period comparison (delta) — Customer Margin doesn't (yet)
...
| Component | Type | Key Metrics | Thresholds |
|-----------|------|-------------|------------|
| `sm_top_bottom` | Chart | Top/bottom 10 suppliers + items, stars, concentration | Top 1 >15% GP = bad, Top 10 >60% = bad |
...
**Action items:**
- Combine 6 KPIs into one "P&L Summary" component (full waterfall)
...
| # | Page | Action |
|---|------|--------|
| 1 | Payment | Rename DSO → Collection Days (code, SQL, UI) |
| 2 | Payment | Change `payment_collection_trend` tool policy: `none` → `aggregate_only` |
| 3 | Payment | `customer_credit_health`: read weights/thresholds from `app_settings` |
| 4 | Sales | Combine `net_sales` + `invoice_sales` + `cash_sales` into one component |
| 5 | Customer Margin | Add prior-period delta to overview KPIs |
| 6 | Financial | Combine 6 overview KPIs into one "P&L Summary" component |
