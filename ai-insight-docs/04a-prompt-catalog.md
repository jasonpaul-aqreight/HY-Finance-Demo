...
> **Classification:** Domain Pack (reference companion to 04)
> **Enables:** Exact reproduction of every prompt the finance pack sends — the two system prompts and all 69 component bodies.
> **Read after:** 00, 02, 04

---

## 1. Purpose & how to use this catalog

Doc 04 specifies *how* prompts are assembled and rendered. This catalog is *what* is assembled: the verbatim, source-exact prompt text. It is a reference index, not an eight-part layer document.

A component's actual model input is produced **mechanically** (doc 04 §5.2):

```
component user message =
  renderThresholdText(<verbatim component template below>, componentKey)
  + "\n\nPage: <page>\nSection: <section>\nComponent: <name> (<type>)\n<scope line>"
  + "\n\nCurrent Values:\n" + <threshold-rendered fetched data block>
system message = the Global Component System Prompt (§2.1)
```

`renderThresholdText` replaces every `{{component_key.token}}` with its value from the **doc 02 Threshold Registry** (defaults, or the DB/override per doc 02 §5.3). Tokens are **defined in doc 02** — this catalog only shows where they sit in the prose. A template with no `{{…}}` renders unchanged.

Because rendering is 100% mechanical, this catalog gives each template **verbatim with tokens in place** (the artifact you must reproduce exactly) plus one fully-worked rendered example per page (§5) to demonstrate substitution. Reproduce template + apply the doc 02 values = the exact sent prompt.

## 2. System prompts (verbatim, applied to every call)

### 2.1 Global Component System Prompt

Sent as the `system` message for **every** per-component call (doc 04 §5.2). No threshold tokens.
...
### 2.2 Summary System Prompt

Sent as the `system` message for the section summary agent loop (doc 04 §5.3–5.4). No threshold tokens; defines the `===INSIGHT===` output grammar the parser (doc 04 §5.6) expects.
...
## 3. Entry schema (shared)

Every component entry below carries this fixed schema. Fields that are identical for all components are stated once here and referenced as **(shared)**:

- **ID** — the `componentKey` (the `DEFAULT_COMPONENT_PROMPTS` map key; also the prompt-config key and the threshold-token namespace).
- **Trigger** — the section it belongs to and that section's scope. Scope is owned by the **doc 02** catalog (`SECTION_SCOPE`); a component is generated whenever its section is in the active batch. Prompts that hard-state their own scope (e.g. "SNAPSHOT, NOT date-filtered") are noted in **Edge cases**.
- **Inputs** — the fetched data block (`Current Values:`), produced by that component's fetcher (doc 02 owns the per-component data contract; doc 04 §5.2 owns the fetch mechanism). The template's own "Data:" line, where present, enumerates what the block contains.
- **Verbatim template** — exact source text, tokens in place. **(the artifact to reproduce)**
- **Rendered example** — *(shared rule)* substitute each `{{key.token}}` with its doc 02 value; assemble per §1. One full worked example per page in §5.
- **Output shape** — *(shared)* the Global Component System Prompt format (§2.1): a `**Current Status:**` line with an alert tag, then `**Key Observations**` bullets. Components add no per-component output schema; their templates only steer *what to look at*.
- **Edge cases** — per-component bans/"hard rules" carried in the template (e.g. "do NOT recompute", sub-period averaging bans, snapshot/date-filter notes). Where none, "none beyond the shared rules in §2.1".

## 4. Master index (69 components · 7 pages · 16 sections)

Tool policy is the section's `ToolPolicy` (doc 04 §5.5): `agg` = `aggregate_only`, `full` = both tools.

| Page | Section (key) | Tool | Components (IDs) |
|---|---|---|---|
| Payment | Payment Collection Trend (`payment_collection_trend`) | agg | avg_collection_days, collection_rate, avg_monthly_collection, collection_days_trend, invoiced_vs_collected |
| Payment | Outstanding Payment (`payment_outstanding`) | full | total_outstanding, overdue_amount, credit_limit_breaches, aging_analysis, credit_usage_distribution, customer_credit_health |
| Sales | Sales Trend (`sales_trend`) | agg | net_sales, invoice_sales, cash_sales, credit_notes, net_sales_trend |
| Sales | Sales Breakdown (`sales_breakdown`) | full | by_customer, by_product, by_agent, by_outlet |
| Customer Margin | Customer Margin Overview (`customer_margin_overview`) | agg | cm_net_sales, cm_cogs, cm_gross_profit, cm_margin_pct, cm_active_customers, cm_margin_trend, cm_margin_distribution |
| Customer Margin | Customer Margin Breakdown (`customer_margin_breakdown`) | full | cm_top_customers, cm_customer_table, cm_credit_note_impact |
| Supplier Performance | Supplier Margin Overview (`supplier_margin_overview`) | agg | sp_net_sales, sp_cogs, sp_gross_profit, sp_margin_pct, sp_active_suppliers, sp_margin_trend, sp_margin_distribution |
| Supplier Performance | Supplier Margin Breakdown (`supplier_margin_breakdown`) | full | sm_top_bottom, sm_supplier_table, sm_item_pricing, sm_price_scatter |
| Returns | Return Trends (`return_trend`) | agg | rt_total_returns, rt_settled, rt_unsettled, rt_return_pct, rt_settlement_breakdown, rt_monthly_trend, rt_product_bar |
| Returns | Unsettled Returns (`return_unsettled`) | full | ru_aging_chart, ru_debtors_table |
| Expenses | Expense Overview (`expense_overview`) | agg | ex_total_costs, ex_cogs, ex_opex, ex_yoy_costs, ex_cost_trend, ex_cost_composition, ex_top_expenses |
| Expenses | Expense Breakdown (`expense_breakdown`) | full | ex_cogs_table, ex_opex_table |
| Financial | Financial Overview (`financial_overview`) | agg | fin_pnl_summary, fin_monthly_trend |
| Financial | Profit & Loss Detail (`financial_pnl`) | agg | fin_pl_statement, fin_yoy_comparison |
| Financial | Balance Sheet (`financial_balance_sheet`) | agg | bs_trend, bs_statement |
| Financial | Variance, Forecast & Budget (`financial_variance`) | agg | fv_variance_summary, fv_variance_breakdown, fv_trend_forecast, fv_budget_suggestions |

*HR sections (`employee_demographics`, `attendance_leave`, `overtime_work_hours`, `payroll_compensation`, `performance_talent`) exist in the catalog as a scaffold with **no component prompts and tool policy `none`** — they are not part of this finance pack and produce nothing until an HR Domain Pack is authored (doc 10).*

## 5. Catalog entries

Each entry: **ID** · type · section · tool policy, the **verbatim template**, and **Edge cases**. Output shape and the rendering rule are **(shared)** per §3. The first entry of each page also carries a full **rendered example**.

---

### 5.1 Page: Payment

#### Section `payment_collection_trend` — tool `aggregate_only`

**`avg_collection_days`** · kpi
...
*Edge cases:* none beyond §2.1.

> **Rendered example (worked, page-representative).** With doc 02 defaults `avg_collection_days.good_days = 45`, `avg_collection_days.warning_days = 60` (illustrative — use the live doc 02 values), and scope = a calendar range, the assembled **component user message** is:
>
> ```text
> "Avg Collection Days" KPI — average days to collect payment after invoicing.
>
...
> Average Payment Speed Rules:
> - Good: 0-45 days
> - Warning: >45 to 60 days
> - Critical: >60 days
>
> Page: Payment
> Section: Payment Collection Trend
> Component: Avg Collection Days (kpi)
> Date Range: 2025-01-01 to 2025-03-31
>
> Current Values:
> <threshold-rendered fetched data block>
> ```
> The `system` message is §2.1 verbatim. Every other component below assembles identically — template, then the `Page/Section/Component/<scope line>` header, then `Current Values:` + data.
...
*Edge cases:* no threshold tokens; "No fixed threshold" — evaluate by trend.

**`collection_days_trend`** · chart
...
*Edge cases:* references another component's token (`avg_collection_days.good_days`) — cross-component tokens are valid (doc 02 renders globally).

**`invoiced_vs_collected`** · chart
...
*Edge cases:* **sub-period averaging banned**; quote pre-computed fiscal-quarter lines verbatim; no mental arithmetic.

#### Section `payment_outstanding` — tool `full`
...
*Edge cases:* **snapshot — ignores the date range**; no fixed threshold.
...
*Edge cases:* may use tools (section policy `full`) to name breaching customers.

**`aging_analysis`** · chart
...
**`credit_usage_distribution`** · chart
...
*Edge cases:* treat resolved `risk_tier`/`credit_score` as authoritative — **do not reverse-engineer the formula**; customers with no credit limit are a mandatory negative flag; do not list every customer.

---

### 5.2 Page: Sales

#### Section `sales_trend` — tool `aggregate_only`
...
*Edge cases:* none beyond §2.1. *(Rendered example: per §3 shared rule — substitute `net_sales.*` tokens with doc 02 values, assemble per §1, exactly as the §5.1 worked example.)*
...
*Edge cases:* no threshold tokens.
...
#### Section `sales_breakdown` — tool `full`
...
### 5.3 Page: Customer Margin

#### Section `customer_margin_overview` — tool `aggregate_only`
...
*Edge cases:* none beyond §2.1. *(Rendered example: shared rule §3 — substitute `cm_net_sales.*` and assemble per §1.)*
...
*Edge cases:* **never analyse COGS in isolation** — always frame vs Net Sales.
...
*Edge cases:* no threshold tokens (directional rules).
...
*Edge cases:* monthly granularity only; cite months from the data.

**`cm_margin_distribution`** · chart
...
*Edge cases:* fixed buckets; population floor RM 1,000 revenue.

#### Section `customer_margin_breakdown` — tool `full`

**`cm_top_customers`** · chart
...
*Edge cases:* cover both lists (GP and Margin%); name star accounts.

**`cm_customer_table`** · table
...
**`cm_credit_note_impact`** · table

```text
"Credit Note Impact on Margins" table — customers ranked by margin lost from credit notes. Columns: Code, Name, Invoice Rev, CN Rev, Return Rate %, Margin Before CN, Margin After CN, Margin Lost %.
...
### 5.4 Page: Supplier Performance

#### Section `supplier_margin_overview` — tool `aggregate_only`
...
*Edge cases:* "Est." = supplier-margin pre-compute, not raw invoices. *(Rendered example: shared rule §3.)*
...
*Edge cases:* **never call rising COGS "bad" in isolation** (supplier-page framing); no threshold tokens.
...
*Edge cases:* reuses `sp_net_sales.good_growth_pct` (cross-component token).
...
*Edge cases:* shrinking is not automatically bad (supplier-page framing).

**`sp_margin_trend`** · chart

```text
"Profitability Trend" chart — monthly bars = Est. Gross Profit (RM), line = Margin % (right axis). Monthly only.
...
*Edge cases:* monthly only; cite months.

**`sp_margin_distribution`** · chart
...
*Edge cases:* analyse **both** Suppliers and Items distributions — do not assume one view.

#### Section `supplier_margin_breakdown` — tool `full`
...
*Edge cases:* cite named suppliers and items.

**`sm_supplier_table`** · table
...
*Edge cases:* always name loss-makers.

**`sm_item_pricing`** · breakdown
...
*Edge cases:* **anchor-item specific — do NOT generalize**; cite suppliers by name.

**`sm_price_scatter`** · chart
...
*Edge cases:* cite items by name.

---

### 5.5 Page: Returns

#### Section `return_trend` — tool `aggregate_only`

**`rt_total_returns`** · kpi
...
*Edge cases:* period flow, not point-in-time. *(Rendered example: shared rule §3.)*
...
*Edge cases:* domain rule — knock-off preferred over refund.
...
*Edge cases:* domain rule — knock-off preferred; unsettled = broken process.

**`rt_monthly_trend`** · chart
...
*Edge cases:* use month names and roll-ups from data only.

**`rt_product_bar`** · chart
...
*Edge cases:* cover both frequency and value lists; name items on both.

#### Section `return_unsettled` — tool `full`

**`ru_aging_chart`** · chart
...
*Edge cases:* **SNAPSHOT, cumulative, NOT date-filtered.**

**`ru_debtors_table`** · table
...
*Edge cases:* **SNAPSHOT, cumulative, NOT date-filtered**; name top 5 verbatim.

---

### 5.6 Page: Expenses

#### Section `expense_overview` — tool `aggregate_only`
...
*Edge cases:* none beyond §2.1. *(Rendered example: shared rule §3.)*
...
*Edge cases:* defer the margin call to the sales-page cross-check.
...
*Edge cases:* name the structural driver if growth is high.
...
*Edge cases:* cite months/values from data — do not invent.

**`ex_cost_composition`** · chart
...
*Edge cases:* **do not recompute %.**

**`ex_top_expenses`** · chart
...
*Edge cases:* name accounts verbatim; **do not change acc_no.**

#### Section `expense_breakdown` — tool `full`
...
*Edge cases:* **do NOT compare to prior year**; name accounts verbatim from top-10.
...
*Edge cases:* **do NOT compare to prior year**; name categories/accounts verbatim.

---

### 5.7 Page: Financial

*All four Financial sections are `aggregate_only` and use **fiscal-period scope** (doc 02 `SECTION_SCOPE`): the scope line reads `Fiscal Period: FY… (fy|last12|ytd)`.*

#### Section `financial_overview` — tool `aggregate_only`
...
*Edge cases:* fiscal-window scope. *(Rendered example: shared rule §3 — note the scope line is `Fiscal Period: …`.)*

**`fin_monthly_trend`** · chart
...
*Edge cases:* **do NOT compute averages over arbitrary sub-windows**; cite pre-calculated roll-ups.

#### Section `financial_pnl` — tool `aggregate_only`

**`fin_pl_statement`** · table
...
*Edge cases:* cite only top-5 mover names; **do NOT recompute YoY %.**

**`fin_yoy_comparison`** · table
...
*Edge cases:* exclude partial FYs from CAGR/trend; no recompute; don't overclaim streaks.

#### Section `financial_balance_sheet` — tool `aggregate_only`
...
*Edge cases:* no sub-window recompute; negative-equity months called out by name.

**`bs_statement`** · table

```text
Full Balance Sheet Statement, selected FY vs 12 periods prior (YTD-aligned). 8 line items by acc_type (Fixed Assets, Other Assets, Current Assets, Current Liabilities, LT Liabilities, Other Liabilities, Capital, Retained Earnings) + derived totals + solvency ratios.
...
*Edge cases:* cite only top-3 mover names; **do NOT recompute YoY % or ratios.**
...
*Edge cases:* fixed thresholds (no tokens); strict budget-labelling rules; one hedged advisory sentence on Material/Severe; **do NOT recompute variance %.**

**`fv_variance_breakdown`** · table
...
*Edge cases:* fixed thresholds (no tokens); cite only pre-fetched names; **do NOT recompute RM or %.**

**`fv_trend_forecast`** · kpi
...
*Edge cases:* fixed thresholds (no tokens); **EXPLAIN, do NOT generate** forecasts; disclaim as AI estimates; conditional budget-block rules.

**`fv_budget_suggestions`** · kpi
...
*Edge cases:* no threshold tokens; suggestions pre-computed — **do NOT invent numbers**; conditional baseline-labelling rules.

## 6. Reproduction checkpoint

**Definition of Done:** a developer with doc 02 (token values), doc 04 (assembly mechanism) and this catalog — no source access — can, for any of the 69 components, produce byte-for-byte the prompt the engine sends:

1. Take the component's verbatim template from §5.
2. Apply `renderThresholdText` with the doc 02 Threshold Registry values (defaults, or DB/override per doc 02 §5.3); templates with no `{{…}}` are unchanged.
3. Append the `Page/Section/Component/<scope line>` header and `Current Values:` block, with the Global Component System Prompt (§2.1) as system — exactly as the §5.1 worked example.
4. For the summary call, every component's rendered template appears as its `About:` block under the Summary System Prompt (§2.2), per doc 04 §5.3.

Verify by diffing a rendered entry against the model input captured with the debug log (doc 04 §5.7) for the same component: they must be identical.
